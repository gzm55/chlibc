...
#define RELO_SET_OFFSET(p, n)                                                                     \
  __extension__({                                                                                 \
    ...
    if (offsetof(loader_relo_types, n) < sizeof(void*))                                           \
      ...
      _param->written = align_u(_param->written, __RELO_ALIGNOF(n));                              \
      _param->relo_offsets[offsetof(loader_relo_types, n) / sizeof(void*) - 1] = _param->written; \
    }                                                                                             \
    (__RELO_TYPE_UQ(n)) & _param->data[_param->written];                                          \
  })
...
#define LOADER_PARAM_CHLIBC_PATH_OFS(p) (sizeof(loader_param_t) + __RELO_OFS(p, chlibc_path))

typedef struct {
  alignas(STACK_ALIGNAS) struct {
    union {
      common_regs_t regs;
      uint32_t written;  // written and data makes a tiny heap on stack
    };
...
extern const uint8_t trap_restore_marker[];
extern const uint8_t trap_ok_marker[];

#ifdef ARCH_X64
void loader_fix_stack(loader_param_t*, long, char**);
#else
void loader_fix_stack(loader_param_t*, char**);
#endif
...
stack_move_info_t loader_main(loader_param_t*, uint64_t, loader_reg_flags_t);
